...
const agents = [
  {
    ...
    detail: 'The Recruiter Agent uses advanced NLP to parse candidate responses in real-time, generating follow-up questions that probe deeper into areas of expertise. It maintains a dynamic difficulty curve, ensuring every interview is both challenging and fair. With access to over 50,000 industry-specific question templates, it tailors each session to the exact role requirements.',
  },
  {
    id: 2,
    name: 'Emotion Agent',
    ...
    detail: 'Our Emotion Agent leverages computer vision and audio processing to detect 12+ distinct emotional states. It tracks confidence levels, nervousness indicators, and engagement metrics throughout the interview. The resulting emotional intelligence profile helps hiring managers understand not just what candidates say, but how they feel about their answers.',
  },
  {
    id: 3,
    name: 'Resume Agent',
    ...
    accentColor: '#22c55e',
    stats: [
      { label: 'ATS Score', value: '99%' },
      { label: 'Skills Parsed', value: '200+' },
    ],
    capabilities: ['Skill Extraction', 'Gap Analysis', 'ATS Optimization'],
    detail: 'The Resume Agent processes documents in 40+ formats, extracting structured data from even the most creative layouts. It cross-references skills against current market demand, identifies transferable competencies, and generates an ATS compatibility score. Candidates receive actionable suggestions to optimize their profiles for both human reviewers and automated screening systems.',
  },
  {
    id: 4,
    name: 'Career Agent',
    ...
    detail: 'The Career Agent synthesizes data from thousands of successful career trajectories to build personalized growth plans. It identifies skill gaps relative to target roles, recommends learning resources, and surfaces opportunities that align with both current abilities and long-term aspirations. Its matching algorithm considers culture fit, growth potential, and compensation benchmarks.',
  },
];
...
function getStackState(relIdx: number) {
  const visible = relIdx < MAX_VISIBLE;
  const mult = relIdx === 0 ? 1 : 1.1;
  const x = visible ? (relIdx * 28) / mult : (MAX_VISIBLE - 1) * 28;
  const scale = visible
    ? 1 / (MAX_VISIBLE / (MAX_VISIBLE * mult - relIdx))
    : 1 / MAX_VISIBLE;
  const opacity = visible ? 1 / (MAX_VISIBLE / (MAX_VISIBLE - relIdx)) : 0;
  const shadow =
    relIdx === 0 ? "0 10px 40px rgba(0,0,0,.15)" : "0 0px 0px rgba(0,0,0,.2)";
  return { x, scale, opacity, shadow };
}

function buildSprings(total: number, cur: number, overrides?: Partial<CardSpring>[]): CardSpring[] {
  return Array.from({ length: total }, (_, i) => {
    ...
    return {
      ...base,
      ...(overrides?.[i] ?? {}),
      zIndex: total - (rel < 0 ? rel + total : rel),
...
function useCardStack(
  ...
  const [springs, setSprings] = useState<CardSpring[]>(() => buildSprings(total, 0));
  ...
  const rebuild = useCallback(
    (cur: number) => setSprings(buildSprings(total, cur)),
    [total]
  );

  const liveUpdate = useCallback((cur: number, dx: number) => {
    setSprings(
      Array.from({ length: total }, (_, i) => {
        const rel = i - cur;
        if (rel < 0) return { ...getStackState(999), zIndex: total + rel, pointerEvents: "none" as const };
        if (rel === 0) {
          const base = getStackState(0);
          return { ...base, x: base.x + dx, zIndex: total, pointerEvents: "auto" as const };
        }
        ...
        return {
          ...
          scale: s0.scale + (s1.scale - s0.scale) * ratio,
          opacity: s0.opacity + (s1.opacity - s0.opacity) * ratio,
          shadow: s0.shadow,
          zIndex: total - rel,
  ...
  const onPointerUp = useCallback((e: React.PointerEvent) => {
    ...
    if (dx < -80 && cur < total - 1) {
      curRef.current = cur + 1;
      rebuild(cur + 1);
      onNavigate(cur + 1);
    } else if (dx > 24 && cur > 0) {
      curRef.current = cur - 1;
...
// ── 3. ANIMATED HEADER ────────────────────────────────────────────────────────
function AnimatedHeader({ agent, direction }: { agent: typeof agents[0]; direction: 1 | -1 }) {
    ...
    <div style={{ position: "relative", height: 64, overflow: "hidden" }}>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <h1 style={{ fontFamily: "'Sora', sans-serif", fontSize: 24, fontWeight: 800, margin: 0, lineHeight: 1.4, flex: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", paddingRight: 16, color: '#0c1b2e' }}>
            {displayed.name}
          </h1>
          <span style={{ fontSize: 13, fontWeight: 700, color: displayed.accentColor, whiteSpace: "nowrap" }}>Active</span>
        </div>
        <address style={{ fontFamily: "'Sora', sans-serif", fontSize: 13, fontWeight: 600, fontStyle: "normal", display: "flex", alignItems: "center", gap: 6, marginTop: 2, color: '#6b8fad' }}>
          {displayed.role}
        </address>
      </div>
      <style>{`
...
// ── 4. DETAIL SHEET ───────────────────────────────────────────────────────────
function DetailSheet({ agent, isOpen, onClose }: { agent: typeof agents[0]; isOpen: boolean; onClose: () => void }) {
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 20, pointerEvents: isOpen ? "auto" : "none" }}>
      <div
        onClick={onClose}
        style={{ position: "absolute", inset: 0, background: isOpen ? "rgba(0,0,0,0.38)" : "rgba(0,0,0,0)", transition: "background .4s ease" }}
      />
      {/* Back button */}
          ...
          <div style={{ padding: "16px 24px 32px" }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '20px' }}>
              {agent.capabilities.map(cap => (
                <span key={cap} style={{ fontFamily: "'Sora', sans-serif", fontSize: 11, fontWeight: 600, padding: '4px 10px', borderRadius: '99px', background: '#f0f4f8', color: '#0c1b2e' }}>
                  {cap}
                </span>
              ))}
            </div>
    ...
    <div style={{ background: "#fff", boxShadow: "0 10px 40px rgba(12,27,46,.08)", width: "100%", maxWidth: 380, borderRadius: 24, overflow: "hidden", display: "flex", flexDirection: "column", height: 520 }}>
      <section style={{ height: "100%", position: "relative", flex: 1, display: "flex", flexDirection: "column", paddingTop: 20, overflow: "hidden" }}>
        ...
        <nav style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 24px", marginBottom: 6, opacity: isOpen ? 0 : 1, pointerEvents: isOpen ? "none" : "auto", transition: "opacity .3s" }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: '#6b8fad', fontFamily: "'Sora', sans-serif" }}>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      touchAction: "none",
                      transform: `translate3d(${sp.x}px,0,0) scale(${sp.scale})`,
                      opacity: sp.opacity,
                      zIndex: sp.zIndex,
                      pointerEvents: sp.pointerEvents,
                      transition: isFront ? undefined : "transform .35s cubic-bezier(.4,0,.2,1), opacity .35s",
                  ...
                  >
                    <div style={{
                      ...
                      boxShadow: sp.shadow, transition: "box-shadow .35s",
                      position: 'relative'
                    }}>
                      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.5) 0%, transparent 30%)' }} />
                      <div style={{ position: 'absolute', bottom: 12, left: 0, right: 0, textAlign: 'center', color: '#fff', fontSize: 11, fontWeight: 600, fontFamily: "'Sora', sans-serif", opacity: isFront ? 0.9 : 0 }}>
                        Tap for details
                      </div>
                    </div>
                  </div>
...
export default function OurAgentsSection() {
  return (
    ...
    >
      {/* ═══ Side-by-Side Layout: Cards LEFT, Text RIGHT ═══ */}
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <motion.div
            ...
            className="flex flex-col items-center lg:items-end w-full lg:pr-12"
          >
            <AgentCardApp />
          </motion.div>
          ...
          >
            <div className="agents-badge" style={{ display: 'flex', alignItems: 'center', gap: 8, fontFamily: "'Sora', sans-serif", fontSize: 11, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: '#3d82c4', marginBottom: 24 }}>
              <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#3d82c4' }} />
              Multi-Agent Architecture
            </div>

            <h2
              className="text-[3.5rem] md:text-[4.5rem] lg:text-[5rem] font-bold tracking-tighter leading-[0.95] mb-6"
              style={{ fontFamily: "'Sora', sans-serif" }}
            >
              <span className="text-[#0c1b2e]">Our </span>
              <br />
              <span className="text-[#3d82c4]">Agents</span>
            </h2>

            <p className="text-lg md:text-xl max-w-lg leading-relaxed text-[#6b8fad] mb-10">
              Four specialized AI agents working in harmony — each mastering a critical dimension
              of the hiring process to deliver{' '}
            ...
            <div className="flex gap-12 mb-10">
              <div>
                <div className="text-3xl font-bold text-[#0c1b2e]" style={{ fontFamily: "'Sora', sans-serif" }}>4</div>
                <div className="text-xs font-semibold uppercase tracking-[0.15em] text-[#6b8fad] mt-1" style={{ fontFamily: "'Sora', sans-serif" }}>AI Agents</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-[#0c1b2e]" style={{ fontFamily: "'Sora', sans-serif" }}>97%</div>
                <div className="text-xs font-semibold uppercase tracking-[0.15em] text-[#6b8fad] mt-1" style={{ fontFamily: "'Sora', sans-serif" }}>Accuracy</div>
              </div>
              <div>
                <div className="text-3xl font-bold text-[#0c1b2e]" style={{ fontFamily: "'Sora', sans-serif" }}>24/7</div>
                <div className="text-xs font-semibold uppercase tracking-[0.15em] text-[#6b8fad] mt-1" style={{ fontFamily: "'Sora', sans-serif" }}>Available</div>
              </div>
            </div>
          </motion.div>
